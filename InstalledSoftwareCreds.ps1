...
if ($saveLocation == "screen"){
    ...
    $output = Get-WmiObject Win32_Product -ComputerName $compName -Credential $Credential | select Name,Version | Out-File -Path $logFileLocation
    Write-Output $output | Out-Host
    Pause
}